...
def coverage_plot_v1(protein, peptide_position, output_path, title):
    ...
    ax.tick_params(   # plt.tick_params(
        ...
        labelleft=False) ##True)# labels along the bottom edge are off
...
def vis_psites(output_path,title,length: int, domain_position: Union[list[tuple[int]], None] = None,
...
def coverage_plot_v3(protein, sample_name, start_position_list_1, pep_len_list_1, start_position_list_2, pep_len_list_2, output_path):
    """ generates plot and saves to file """
    ...
    #
    ax1.set_title('peptide coverage   [sample: '+sample_name+']', fontsize=10)#, pad=5
    ...
    #
    p1 = PatchCollection(patches_1, cmap=mpl.cm.jet, alpha=0.5, color='grey') #0.3 ## matplotlib
    p2 = PatchCollection(patches_2, cmap=mpl.cm.jet, alpha=0.5, color='red') #0.3 ## matplotlib
