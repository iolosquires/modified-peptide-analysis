...
def find_chunks_with_phosphosites(seqs, mod_color_dict):
    ...
    chunk_for_plot = [
        item for item, skip in zip(range(0, len(seqs[0])), skip_list) if not skip
    ]
    return chunk_for_plot


def phosphopep_coverage_plot(
    chunk_for_plot,
    chopped_sequences,
    mod_color_dict,
    POI_record,
    ...
    fig, axs = plt.subplots(
        len(chunk_for_plot), 1, figsize=(20, 4 * len(chunk_for_plot))
    )
    ...
    for ax_num, num_seqs in enumerate(chunk_for_plot):
        ...
        for cat, cut, col in zip(categories, cutoffs, colours):
            ...
            axs[ax_num].bar(
                aa_num, pc_mask2, align="edge", linewidth=0, color=col, label=cat
            )
            ...
            for i in range(0, len(protein)):
                if j == 0:
                    axs[ax_num].text(
                        ...
                        sequences[j][i],
                        bbox=dict(
                            facecolor=mod_color_dict.get(aa_num[i] - 1, "grey"),
                            alpha=0.5,
                        ),
                        fontdict=font,
                    )
                    ...
                    axs[ax_num].text(
                        float(aa_num[i] - 1), ypos_xlbl, sequences[j][i], fontdict=font
                    )
...
def fill_pep_groups(pep_groups, groups, POI_record):
    ...
    for group_coords, group_seqs in zip(groups, pep_groups):
        if len(group_coords) == 0:
            continue
        pep_strings.append(
            create_peptide_string(POI_record.seq, group_seqs, group_coords)
        )

    return pep_strings


def master_phosphopeptide_plot(
    ...
    chopped_sequences = create_sequences_for_plot(
        phos_confs, POI_record, pep_strings, chunk_size
    )
    chunk_for_plot = find_chunks_with_phosphosites(chopped_sequences, mod_color_dict)
    phosphopep_coverage_plot(
        chunk_for_plot,
        chopped_sequences,
        mod_color_dict,
        POI_record,
...
def create_phospho_peptide_plot(
